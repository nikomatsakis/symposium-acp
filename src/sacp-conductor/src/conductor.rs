...
use sacp::schema::{
    AgentRole, ClientRole, ConductorRole, McpConnectRequest, McpConnectResponse,
    McpDisconnectNotification, McpOverAcpMessage, SuccessorMessage,
};
use sacp::{Component, schema::ProxyRole};
use sacp::{Error, JrMessage};
use sacp::{
    JrConnectionCx, JrHandlerChain, JrNotification, JrRequest, JrRequestCx, JrResponse, JrRole,
    MessageCx, MetaCapabilityExt, Proxy, UntypedMessage, UntypedRole,
};
use sacp::{
...
struct ConductorHandlerState {
    ...
    proxies: Vec<JrConnectionCx<ConductorRole, ProxyRole>>,
    ...
    ///
    /// Populated lazily when the first Initialize request is received.
    agent: Option<JrConnectionCx<ConductorRole, AgentRole>>,
...
impl JrMessageHandler for ConductorMessageHandler {
    type Local = ConductorRole;
    type Remote = ClientRole;
    type Counterpart = ClientRole;

    async fn handle_message(
        &mut self,
        message: MessageCx<ConductorRole, ClientRole>,
    ) -> Result<sacp::Handled<MessageCx<ConductorRole, ClientRole>>, sacp::Error> {
        JrHandlerChain::new(ConductorRole, ClientRole)
            .on_receive_message_from(AgentRole, {
                let mut conductor_tx = self.conductor_tx.clone();
                async move |message: MessageCx<ConductorRole, ClientRole>| {
            ...
            .on_receive_message_from(ClientRole, {
                let mut conductor_tx = self.conductor_tx.clone();
                async move |message: MessageCx<ConductorRole, ClientRole>| {
                    conductor_tx
...
impl ConductorHandlerState {
    ...
    fn extract_trace_info<R: sacp::JrRequest, N: sacp::JrNotification>(
        message: &sacp::MessageCx<UntypedRole, UntypedRole, R, N>,
        ...
        match message {
            sacp::MessageCx::Request(request, _) => {
                ...
                if let Some(Ok(mcp_req)) = <McpOverAcpMessage<UntypedMessage>>::parse_request(
                    &untyped.method,
                    &untyped.params,
                ) {
                    return Ok((
            ...
            sacp::MessageCx::Notification(notification, _) => {
                let untyped = notification.to_untyped_message()?;

                // Try to parse as MCP-over-ACP notification
                if let Some(Ok(mcp_notif)) = <McpOverAcpMessage<UntypedMessage>>::parse_notification(
                    &untyped.method,
                    &untyped.params,
                ) {
                    return Ok((
    ...
    async fn handle_conductor_message(
        &mut self,
        client: &JrConnectionCx<ConductorRole, ClientRole>,
    ...
    fn send_message_to_predecessor_of<Req: JrRequest, N: JrNotification>(
        ...
        message: MessageCx<UntypedRole, UntypedRole, Req, N>,
    ) -> Result<(), sacp::Error>
    where
        Req::Response: Send,
    {
        ...
        match message {
            ...
            MessageCx::Notification(notification, _cx) => self
    ...
    fn send_notification_to_predecessor_of<N: JrNotification>(
        ...
        if source_component_index == 0 {
            ...
            self.proxies[source_component_index - 1].send_notification(SuccessorNotification {
                notification,
                meta: None,
            })
        }
    ...
    async fn forward_client_to_agent_message(
        ...
        let Some(message) = self
            .ensure_initialized(conductor_tx, client, message)
            .await?
        else {
            return Ok(());
        };
    ...
    async fn forward_initialize_request(
        ...
        target_component_index: usize,
        conductor_tx: &mpsc::Sender<ConductorMessage>,
        cx: &JrConnectionCx<ConductorRole, ClientRole>,
        ...
        if initialize_req.has_meta_capability(Proxy) {
            // Proxy components should never forward initialize with proxy capability attached
                ...
                .await_when_result_received(async move |response| {
                    ...
                    match response {
                        Ok(mut response) => {
                            ...
                            // Remove the proxy capability from the response,
                            // that is just targeted us (the conductor) and shouldn't be sent
    ...
    async fn ensure_initialized(
        ...
        client: &JrConnectionCx<ConductorRole, ClientRole>,
        message: MessageCx<ConductorRole, ClientRole>,
    ) -> Result<Option<MessageCx<ConductorRole, ClientRole >>, Error> {
        ...
        match message {
            MessageCx::Request(request, request_cx) => {
                match InitializeRequest::parse_request(request.method(), request.params()) {
                    Some(Ok(init_request)) => {
                    ...
                    None => {
                        request_cx.respond_with_error(
                            Error::invalid_request().with_data("expected an `initialize` request"),
                        )?;
                        Ok(None)
                    }
                }
            ...
            MessageCx::Notification(_, notification_cx) => {
                notification_cx.send_error_notification(
                    Error::invalid_request().with_data("expected an `initialize` request"),
                )?;
                Ok(None)
            }
    ...
    async fn lazy_initialize_components(
        ...
        initialize_request_cx: JrRequestCx<ConductorRole, ClientRole, InitializeResponse>,
    ) -> Result<
        (
            InitializeRequest,
            JrRequestCx<ConductorRole, ClientRole, InitializeResponse>,
        ),
        sacp::Error,
    > {
        assert!(self.proxies.is_empty());
        ...
        let proxy_mode = initialize_request.has_meta_capability(Proxy);
        let initialize_request = initialize_request.remove_meta_capability(Proxy);
        ...
        if !proxy_mode {
            ...
            let agent_cx = cx.spawn_connection(
                JrHandlerChain::new(ConductorRole, AgentRole)
        ...
        for (component_index, dyn_component) in dyn_components.into_iter().enumerate() {
            ...
            let proxy_cx = cx.spawn_connection(
                JrHandlerChain::new(ConductorRole, ProxyRole)
                        ...
                        >| {
                            conductor_tx
                                .send(ConductorMessage::ClientToAgent {
                                    ...
                                    message: message_cx
                                        .map(|r, cx| (r.message, cx), |n, cx| (n.message, cx)),
                                })
                                .await
                                .map_err(sacp::util::internal_error)
                        }
        ...
        Ok((
            modified_req,
            initialize_request_cx.wrap_params(move |_method, response| {
                let response: InitializeResponse = response?;
                if proxy_mode {
                    Ok(response.add_meta_capability(Proxy))
                } else {
                    Ok(response)
                }
            }),
    ...
    async fn forward_session_new_request(
        ...
        let Some(target_cx) = self
            .proxies
            .get(target_component_index)
            .or_else(|| self.agent.as_ref())
        else {
            // Forwarding a message to the conductor's successor SHOULD be handled by caller,
            // so this should be unreachable.
            return Err(sacp::util::internal_error("invalid target component index"));
        };
...
trait JrConnectionCxExt<Local: JrRole, Remote: JrRole> {
...
impl<Local: JrRole, Remote: JrRole> JrConnectionCxExt<Local, Remote>
    for JrConnectionCx<Local, Remote>
where
    Local: sacp::HasRemoteRole<Remote>,
{
    fn send_proxied_message_via(
        &self,
        conductor_tx: &mpsc::Sender<ConductorMessage>,
        ...
        match message {
            MessageCx::Request(request, request_cx) => self
                .send_request(request)
                .forward_response_via(conductor_tx, request_cx),
            MessageCx::Notification(notification, _) => self.send_notification(notification),
        }
    }
}

trait JrRequestCxExt<Local: JrRole, Remote: JrRole, T: JrResponsePayload> {
...
impl<Local: JrRole, Remote: JrRole, T: JrResponsePayload> JrRequestCxExt<Local, Remote, T>
...
pub trait JrResponseExt<Local: JrRole, Remote: JrRole, T: JrResponsePayload> {
    fn forward_response_via<L2: JrRole, R2: JrRole>(
        self,
        conductor_tx: &mpsc::Sender<ConductorMessage>,
...
impl<Local: JrRole, Remote: JrRole, T: JrResponsePayload> JrResponseExt<Local, Remote, T>
    for JrResponse<Local, Remote, T>
{
    fn forward_response_via<L2: JrRole, R2: JrRole>(
        self,
        conductor_tx: &mpsc::Sender<ConductorMessage>,
